...
function parser() {
  ...
  async function parserGet() {
    ...
    await k.apiParserGetAllParserPageGet(opts).then(async response => {
  ...
  async function parserDelete(id) {
    Swal.fire({
      title: 'Are you sure?',
      text: 'You want to delete this Parser?',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        const k = new ParserApi();
        k.apiParserIdDelete(id)
          .then(async response => {
            if (response.body.message === 'Deleted Successfully.') {
              Swal.fire({
                icon: 'success',
                title: 'Delete',
                text: 'Parser Deleted Successfully !',
              });
              parserGet();
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: params => (
        <>
          <Tooltip title="View Parser" arrow placement="right">
            <IconButton color="primary" size="small">
              <Link
                href={{
                  pathname: `${config.masterRoutes.parser}/view`,
          ...
          <Tooltip title="Edit Parser" arrow placement="right">
            <IconButton color="primary" size="small">
              <Link
                href={{
          ...
          <Tooltip title="Delete Parser" arrow placement="right">
            <IconButton
              color="primary"
              onClick={() => {
                parserDelete(params.row.id);
              }}
              size="small"
            >
              <DeleteIcon
  ...
  return (
    <MainCard
      title="Parser"
      secondary={
        <Tooltip title="Add Parser" arrow placement="right">
