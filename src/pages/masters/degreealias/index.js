...
function degreealias() {
  ...
  async function degreealiasGet() {
    ...
    await k.apiDegreeAliasGetAllByPageGet(opts).then(async response => {
  ...
  async function DegreeAliasDelete(id) {
    Swal.fire({
      title: 'Are you sure?',
      text: 'You want to delete this Degree Alias?',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        const k = new DegreeAliasApi();
        k.apiDegreeAliasIdDelete(id)
          .then(async response => {
            if (response.body.message === 'Deleted Successfully.') {
              Swal.fire({
                icon: 'success',
                title: 'Delete',
                text: 'Degree Alias Deleted Successfully !',
              });
              degreealiasGet();
              ...
              Swal.fire({
                icon: 'error',
                title: 'Oops..',
                text: 'Degree Alias Deletion Failed !',
              });
              degreeGet();
            }
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: params => (
        <>
          <Tooltip title="View Degree Alias" arrow placement="right">
            <IconButton color="primary" size="small">
              <Link
                href={{
                  pathname: `${config.masterRoutes.degreealias}/view`,
          ...
          <Tooltip title="Edit Degree Alias" arrow placement="right">
            <IconButton color="primary" size="small">
              <Link
                href={{
          ...
          <Tooltip title="Delete Degree Alias" arrow placement="right">
            <IconButton
              color="primary"
              onClick={() => {
                DegreeAliasDelete(params.row.id);
              }}
              size="small"
            >
              <DeleteIcon
  ...
  return (
    <MainCard
      title="Degree Alias"
      secondary={
        <Tooltip title="Add Degree Alias" arrow placement="right">
