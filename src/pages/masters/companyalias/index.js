...
function companyalias() {
  ...
  async function CompanyAliasGet() {
    ...
    await k.apiCompanyAliasGetAllByPageGet(opts).then(async response => {
  ...
  async function CompanyAliasDelete(id) {
    Swal.fire({
      title: 'Are you sure?',
      text: 'You want to delete this Company Alias ?',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        const k = new CompanyAliasApi();
        k.apiCompanyAliasIdDelete(id)
          .then(async response => {
            if (response.body.message === 'Deletion Failed.') {
              Swal.fire({
                icon: 'error',
                title: 'Oops..',
                text: 'Company Alias Deletion Failed !',
              });
              CompanyAliasGet();
              ...
              Swal.fire({
                icon: 'success',
                title: 'Delete',
                text: 'Company Alias Deleted Successfully !',
              });
              CompanyAliasGet();
            }
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: params => (
        <>
          <Tooltip title="View Company Alias " arrow placement="right">
            <IconButton color="primary" size="small">
              <Link
                href={{
                  pathname: `${config.masterRoutes.companyalias}/view`,
          ...
          <Tooltip title="Edit Company Alias " arrow placement="right">
            <IconButton color="primary" size="small">
              <Link
                href={{
          ...
          <Tooltip title="Delete Company Alias " arrow placement="right">
            <IconButton
              color="primary"
              onClick={() => {
                CompanyAliasDelete(params.row.id);
              }}
              size="small"
            >
              <DeleteIcon
  ...
  return (
    <MainCard
      title="Company Alias"
      secondary={
        <Tooltip title="Add Company Alias " arrow placement="right">
