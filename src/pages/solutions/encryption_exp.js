...
const apiObject = {
  getCitynamebyStateId:
    'https://app.razorswift.org/api/City/GetAllByStateAndName',
};
...
const Encryption_exp = () => {
  ...
  const getCustomPathwayURl = useCallback(async () => {
    try {
      const response = await callApi(
        'custom',
        {},
        { stateId: 1, name: '' },
        apiObject['getCitynamebyStateId'],
        'GET'
      );

      console.log(response.data.result, 'TestURL');
